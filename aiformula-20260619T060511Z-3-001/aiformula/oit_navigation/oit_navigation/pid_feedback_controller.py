...
class PIDFeedbackController(Node):
    def __init__(self):
        ...
        self.declare_parameter('Kp_v_st', 0.1)
        self.declare_parameter('Ki_v_st', 0.01)
        self.declare_parameter('Kd_v_st', 0.05)
        
        self.declare_parameter('Kp_w_st', 0.3)
        self.declare_parameter('Ki_w_st', 0.005)
        self.declare_parameter('Kd_w_st', 0.02)
        
        self.declare_parameter('Kp_v_turn', 0.45)
        self.declare_parameter('Ki_v_turn', 0.02)
        self.declare_parameter('Kd_v_turn', 0.04)
        
        self.declare_parameter('Kp_w_turn', 1.0)
        self.declare_parameter('Ki_w_turn', 0.00)
        self.declare_parameter('Kd_w_turn', 0.18)
        
        self.declare_parameter('omega_low', 0.05)
        self.declare_parameter('omega_high', 0.12)
        
        self.declare_parameter('cycle_time', 0.01)
        self.declare_parameter('integrator_limit', 0.5)
        self.declare_parameter('derivative_alpha', 0.7)
        ...
        self.csv_writer.writerow([
            'time[s]', 'v_ref', 'w_ref', 'v_meas', 'w_meas', 'e_v', 'e_w', 'u_v', 'u_w', 'mode'
        ])
    ...
    def control_step(self):
        ...
        if((abs(self.v_ref) < 0.01) and (abs(self.w_ref) < 0.01)):
            self.I_v = 0.0
            self.I_w = 0.0
            
        if (abs(self.prev_ratio - ratio) > 0.5):
            self.I_v = 0.0
            self.I_w = 0.0
            
        self.prev_ratio = ratio
        
        u_v_st = self.Kp_v_st * e_v + self.Ki_v_st * self.I_v + self.Kd_v_st * self.D_v_filt
        u_w_st = self.Kp_w_st * e_w + self.Ki_w_st * self.I_w + self.Kd_w_st * self.D_w_filt
        u_v_turn = self.Kp_v_turn * e_v + self.Ki_v_turn * self.I_v + self.Kd_v_turn * self.D_v_filt
        u_w_turn = self.Kp_w_turn * e_w + self.Ki_w_turn * self.I_w + self.Kd_w_turn * self.D_w_filt
        
        u_v = (1 - ratio) * u_v_st + ratio * u_v_turn
        u_w = (1 - ratio) * u_w_st + ratio * u_w_turn
        ...
        self.csv_writer.writerow([
            round(time(), 3), round(self.v_ref, 3), round(self.w_ref, 3), round(self.v_meas, 3), round(self.w_meas, 3), round(e_v, 3), round(e_w, 3), round(u_v, 3), round(u_w, 3), mode
        ])
